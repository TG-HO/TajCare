...
import { Wrench, Calendar, Clock, MessageSquare, CheckCircle2, Eye, X, Loader2, AlertTriangle, ShieldCheck } from "lucide-react";
...
export default function ResponderClient({
  ...
  function handleOpenActionModal(ticket: Ticket, newStatus: TicketStatus) {
    ...
    setVisitDate(ticket.scheduled_visit_date ? new Date(ticket.scheduled_visit_date).toISOString().slice(0, 16) : "");
    setRemarks("");
  }
      ...
      <div className="bg-[#0F172A] text-white rounded-2xl p-6 shadow-md border border-slate-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-300 mt-1">
            Schedule fueling site visits, update Head Office ticket resolution status, and submit mandatory remarks.
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between bg-white p-4 border border-slate-200 rounded-2xl shadow-sm">
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <option value="Issue Resolved">Resolved</option>
      ...
      <div className="space-y-4">
        {filteredTickets.length === 0 ? (
          ...
          filteredTickets.map((ticket) => {
            ...
            const visitPassed = ticket.scheduled_visit_date && new Date() >= new Date(ticket.scheduled_visit_date);
                  ...
                  <div className="space-y-1 max-w-xl">
                    ...
                    {ticket.scheduled_visit_date && (
                      <div className={`mt-2 inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold border ${
                        visitPassed ? "bg-rose-50 text-rose-700 border-rose-200" : "bg-indigo-50 text-indigo-700 border-indigo-200"
                      }`}>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {!isHeadOffice && (
                      <>
                        ...
                        {/* Reschedule is ONLY available AFTER the visit date has passed (late/SLA breach action) */}
                        {ticket.status === "Visit Date Scheduled" && visitPassed && (
                          <button
                            onClick={() => handleOpenActionModal(ticket, "Visit Date Scheduled")}
                        ...
                        {/* Mark Visited is available when scheduled and visit date has NOT passed yet (on time arrival) OR after date passes */}
                        ...
                        {(ticket.status === "Visited" || ticket.status === "Visit Date Scheduled") && (
                          <button
                            onClick={() => handleOpenActionModal(ticket, "Issue Resolved")}
                            className="px-3 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl shadow transition-all"
                          >
            ...
            <form onSubmit={handleSubmitAction} className="space-y-4 pt-4">
              <div className="p-3 bg-slate-50 border border-slate-200 rounded-xl text-xs space-y-1">
                ...
                <p className="font-bold text-[#0F172A] text-sm">
                  {selectedTicket.status} → <span className="text-emerald-600">{targetStatus}</span>
                </p>
              </div>
